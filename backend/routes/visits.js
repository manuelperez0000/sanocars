...
router.get('/', async (req, res) => {
  try {
    ...
    var [existing] = await db.query('SELECT COUNT(*) as count FROM vicitas')
    if (existing[0].count === 0) {
      // Insert initial record if table is empty
      await db.query('INSERT INTO vicitas (contador) VALUES (1)')
      return responser.success({ res, body: { visits: 1 } })
    }

    // Get the current count, increment it, and return the new value
    var [rows] = await db.query('SELECT contador FROM vicitas LIMIT 1')
    ...
    // Update the counter
    var [result] = await db.query('UPDATE vicitas SET contador = ? WHERE contador = ?', [newCount, rows[0].contador])
