...
async function automatics(conn) {
  ...
  mantenimiento_general text DEFAULT NULL,
  inspeccion_vehicular text DEFAULT NULL,
  ...
  // Add new maintenance columns to existing vehiculos_venta table
  try {
    await conn.query(`ALTER TABLE vehiculos_venta ADD COLUMN IF NOT EXISTS cambio_aceite DATE DEFAULT NULL AFTER trabajos_realizar`);
    console.log("Columna 'cambio_aceite' agregada a tabla 'vehiculos_venta' si no existía.");
  } catch (error) {
    console.log("Columna 'cambio_aceite' ya existe o error al agregar:", error.message);
  }

  try {
    await conn.query(`ALTER TABLE vehiculos_venta ADD COLUMN IF NOT EXISTS mantenimiento_general TEXT DEFAULT NULL AFTER cambio_aceite`);
    console.log("Columna 'mantenimiento_general' agregada a tabla 'vehiculos_venta' si no existía.");
  } catch (error) {
    console.log("Columna 'mantenimiento_general' ya existe o error al agregar:", error.message);
  }

  try {
    await conn.query(`ALTER TABLE vehiculos_venta ADD COLUMN IF NOT EXISTS inspeccion_vehicular TEXT DEFAULT NULL AFTER mantenimiento_general`);
    console.log("Columna 'inspeccion_vehicular' agregada a tabla 'vehiculos_venta' si no existía.");
  } catch (error) {
    console.log("Columna 'inspeccion_vehicular' ya existe o error al agregar:", error.message);
  }

  try {
    await conn.query(`ALTER TABLE vehiculos_venta ADD COLUMN IF NOT EXISTS garantia VARCHAR(255) DEFAULT NULL AFTER inspeccion_vehicular`);
    console.log("Columna 'garantia' agregada a tabla 'vehiculos_venta' si no existía.");
  } catch (error) {
    console.log("Columna 'garantia' ya existe o error al agregar:", error.message);
  }
